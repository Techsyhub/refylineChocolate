...
export default function CheckoutPage() {
  ...
  if (hasTrio) {
    ...
    deliveryDiscount = Math.ceil( DELIVERY_FEE * 0.75);
  } else if (singleQty === 2) {
    deliveryDiscount = Math.ceil(DELIVERY_FEE * 0.75);
  } else if (singleQty >= 3) {
    deliveryDiscount = DELIVERY_FEE;
...
//  console.log("cart first item", cart[0])
    ...
    const payload = {
      ...
      logisticInfo: null,
      subtotal: subtotal,
      deliveryDiscount: Math.ceil(deliveryDiscount)
    };
      ...
      <div className="border rounded-lg p-4 mb-6 bg-white shadow">
        ...
        <div className="space-y-1 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="text-gold">-Rs {Math.ceil(deliveryDiscount)}</span>
          </div>

          <div className="flex justify-between font-bold border-t pt-2">
            <span>Total Payable:</span>
            <span>Rs {Math.ceil(totalAmount)}</span>
          </div>
        </div>
      </div>
